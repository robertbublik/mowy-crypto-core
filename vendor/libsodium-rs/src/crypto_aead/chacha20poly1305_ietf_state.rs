//! ChaCha20-Poly1305-IETF Precomputation State
//!
//! This module provides a precomputation state wrapper for the RFC 8439 / IETF ChaCha20-Poly1305 variant.
//! Like the other AEAD state modules in this crate, this wrapper stores the key so you can reuse
//! it across repeated ChaCha20-Poly1305-IETF operations.
//!
//! ## Features
//!
...
//!
//! ```
//! # use libsodium_rs::crypto_aead::{chacha20poly1305_ietf, chacha20poly1305_ietf_state};
//! # use libsodium_rs::ensure_init;
//! # fn main() -> Result<(), Box<dyn std::error::Error>> {
//! # ensure_init()?;
//! // Create a key and precomputation state
//! let key = chacha20poly1305_ietf::Key::generate();
//! let state = chacha20poly1305_ietf_state::State::from_key(&key)?;
//!
//! // Generate a nonce
...
//! let ciphertext = chacha20poly1305_ietf_state::encrypt_afternm(
//!     message,
...
//!
//! // Decrypt the message
//! let decrypted = chacha20poly1305_ietf_state::decrypt_afternm(
//!     &ciphertext,
//!     Some(additional_data),
//!     &nonce,
//!     &state
...
/// A reusable key wrapper for ChaCha20-Poly1305-IETF encryption and decryption
///
/// This struct mirrors the existing AEAD state-module pattern in this crate by
/// holding a cloned key for repeated operations.
...
/// Encrypt a message using ChaCha20-Poly1305-IETF with a precomputation state
///
/// This function encrypts a message using the ChaCha20-Poly1305-IETF algorithm with a
...
/// Decrypt a message using ChaCha20-Poly1305-IETF with a precomputation state
///
...
/// Encrypt a message using ChaCha20-Poly1305-IETF with detached authentication tag and precomputation state
///
/// This function encrypts a message using the ChaCha20-Poly1305-IETF algorithm with a
...
/// Decrypt a message using ChaCha20-Poly1305-IETF with detached authentication tag and precomputation state
///
/// This function decrypts a message that was encrypted using the ChaCha20-Poly1305-IETF
